...
export const useScrollTop = () => {
    ...
    return [scrollTop, setScrollTop] as const;
}
...
export const useScrollSpeed = () => {
    ...
    let timerRef: NodeJS.Timeout;//useRef(null);
    const delay = 200;
    const timeout = 100;
...
export interface IVirtualScrollProps extends React.ComponentPropsWithoutRef<'div'> {
    items: any[];
    padding: number;
    children: any;
}

export interface IScrollToRowHandle {
    scrollToRowHandle: (rowIndex: number) => void;
}

export const VirtualScroll = forwardRef<IScrollToRowHandle, IVirtualScrollProps>((props, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        scrollToRowHandle(rowIndex) {
            if (rowHeights.length == 0 || rowIndex < 0) {
                return;
            }
